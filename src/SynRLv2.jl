...
This is the second version of the synaptic reinforcement learning library. Primary changes:
 - [x] Cross entropy loss.
 - [x] Matrix copy for each layer of neurons -> faster `forward(net::NeuralNetwork)` function execution.
 	- [x] Change neuron update schema accordingly.
 - [?] Mini-batch stochastic gradient descent (?)
 - [x] Multi-processing for loops.
...
function forward(x, neuron::Neuron)
    return ACTIVATION(transpose(x)*neuron.w)
end
...
function forward(x, network::NeuralNetwork)
	# NOTE: This function uses the `tanh` activation in the final layer, too.
	# One should really use `mass_forward`, which does not do this.
    intermediate = x;
...
function mass_forward(x, network::NeuralNetwork)
	...
	for i in 1:(length(network.layers)-1)
		y = [1; y];
		y = ACTIVATION.(network.layers[i].w_mat*y);
	end
...
function mass_forward_slow(x, network::NeuralNetwork)
    """
    Function for processing a batch of data through a neural network.
    x should be a matrix of column vectors where each column is a single piece of data.
    y will be of the same format with column vectors representing the output.
...
function show_2d_boundary(net::NeuralNetwork; xyrange = 2, num_points = 100, title="Decision Boundary R2 → (1,0)")
	...
	z = mass_forward(xy, net);
	z = sftmx(z);
	z = reshape(z, num_points)
	print("shape of z: ", size(z));

	cutoff = 0.5
    Plots.scatter( (xy[1,:])[z .> cutoff], (xy[2,:])[z .> cutoff], label = "+")
    Plots.scatter!( (xy[1,:])[z .< cutoff], (xy[2,:])[z .< cutoff], label = "-")
    Plots.title!(title)
end
...
function loss_linear(net, x, truth)
	"""
	Simple loss function for en-masse loss calculation of matrix x -> network | truth matrix.
	Calculates euclidian distance between prediction matrices divided by number of total
	predictions (i.e., )
	"""
...
function CE_loss(net, x, truth, softmax::Bool=true)
	"""
	Cross-entropy loss function for matrix comparison.
	By edfault, `softmax` is set to true so each column of the
	output of `mass_forward(x, net)` is put through a softmax function
	before comparison via cross entropy loss.
	"""
	pred = mass_forward(x, net);
	if(softmax==true)
		pred = sftmx(pred)
	end
	return (-1/size(x)[2])*sum(truth.*log.(pred));
end
...
function train_network(x, truth, dim::Int64, iters::Int64, explore_prob::Float64,
    ...
    losses = zeros(N_HIST).+CE_loss(net, x, truth)
	...
	@showprogress 1 "Computing..." for i = 1:num_iterations
		...
		Threads.@threads for l_idx in 1:length(net.layers)
			# for (n_idx, neuron) in enumerate(layer.neurons)
			layer = net.layers[l_idx];
			Threads.@threads for n_idx in 1:length(layer.neurons)
