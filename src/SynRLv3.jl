...
This is the third version of the synaptic reinforcement learning library. Primary changes:
...
activation = "tanh" # name of the activation function. Currently only "relu" and "tanh" are accepted.
```
...
function sftmx(X::Array{Float64})
	"""
...
function CE_loss(net, x, truth, softmax::Bool=true)
	...
	if(softmax==true)
		pred = sftmx(pred .+ 10e-9)
	end

	# println("POST_SFTMX_PRED:");
	# println(pred);
...
#                     addr = [actions[j, end-N_HIST:end-1]; rewards[end-N_HIST:end-1]]
        ...
        new_loss = CE_loss(net, x[:, batch_mask], truth[:, batch_mask]);
...
function get_accuracy(network, x, y)
    y_hat = mass_forward(x, network);
    L = size(y)[2]
    num_correct = 0
    for i = 1:L
